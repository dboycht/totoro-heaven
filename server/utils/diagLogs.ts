 ...
 *
 * ## 取舍：超上限时取**尾部**（最后 `maxBytes` 字节）—— 2026-09-22 闸门复验后**已改**（原为头部）
 * 老实现取文件头，理由写的是"截尾会丢同一天最早发生的线索"。但那是在**没有记录窗口**的老世界里成立的；
 * 现在用户明确是「**开始记录 → 复现 → 导出**」，最新、最有用的证据在**文件尾部**，
 * 截头部恰好把**用户刚复现的那一段**切在包外（闸门复验实测：9MB 日志 + 8MB 上限 ⇒ 最新行被切掉）。
 * 所以改成取尾部，并丢掉可能被切断的**首行残段**（`JSON.parse` 失败的行本来就会被跳过，
 * 但留着会让人以为"这一行坏了"）。截断与否、原始多大，manifest 里都写清楚（note 里写明"保留文件尾"）。
 *
