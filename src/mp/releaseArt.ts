...
export const VERSION_ENTRIES: VersionEntry[] = [
  ...
  },
  {
    ...
    highlights: [
      '⚠️ 本条目**开发中**：更新要点会在发版前统一整理（用户 2026-09-17 要求）。',
      '★ 跑道编辑：**删除"随机道次 + 缓慢换道"**（经实测它是"车道线看着乱"的根因）—— 轨迹**整圈恒定在你所选的那一道**；',
      '★ 所选道次（第 N 道 / 共 M 道）**随本地路线一起保存**，跑步页按它生成，并在界面上显示；',
      '★ 新增**内外圈合法性判定**：外圈必须包着内圈、两圈不许相交、环宽 2~20 m；不合法时**禁止保存并逐条列出问题**；',
      '★ 新增「**按外圈自动生成内圈（向内缩 N 米）**」，只画一条外圈就够；面板显示"车道线离内圈/外圈各多少米"；',
      '🐞 修复：车道线"一头贴外圈、一头贴内圈"（插值改为**法向就近配对**）；「第几道」滑块无效。',
    ],
  ...
  },
  {
    ...
    highlights: [
      '★ 新增「跑道编辑」页：卫星底图上看清跑道，描出**外圈/内圈**，算法在两道之间生成这一趟的**车道线**（随机一道 + 缓慢换道），可一键**快速定位**到任选线路；',
      '★ 「本地路线库」：按线路保存你描好的跑道，列表显示**创建日期与版本**，可随时载入/删除（旧数据自动迁移）；',
      '★ 阳光跑页**只列你配置过的路线**，并按真跑道几何生成轨迹；未配置时仍列官方线路并提示去配置；',
      ...
      '✅ 真实提交实测闭环：轨迹成功入库（云端 pointList 967 点，与提交点数完全吻合）、判定**有效**；确认服务端存的就是客户端上报的拟合度；',
