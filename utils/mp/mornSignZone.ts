...
export interface MornSignZone {
  ...
  /**
   * **⭐ 最该调的一个：进场方向的个人偏置**（度，0=正北，顺时针；`null` = 每次随机方向）。
   *
   * 用户 2026-09-20 明确点名："主要是需要调方向偏置"。为什么它比其它参数重要：
   *
   *   · **距离分布是"围着一个圆"，方向分布是"全向散开"** —— 两者都能从统计上看出"不像人"；
   *     但真人的第一特征是**每次都从同一个方向过来**（宿舍/教学楼在哪个方位，就走哪条路），
   *     所以"方向固定"这一项**最能决定像不像真人**；
   *   · 而**距离**对"像不像"影响小得多：真人站在操场里，离点位中心 40 m 还是 150 m 都正常。
   *
   * ⚠️ 取值要注意：
   *   · `null`（默认）⇒ 每次**全向随机**，落点会围成一个整圆 —— 这是最容易看出是程序的那种；
   *   · 填一个具体方位 ⇒ 落点集中在**该方向的一个扇形**里，像"从那个门走进来站定"；
   *   · 怎么定这个方位：在卫星图上看"你常来的那栋楼/那个门"相对**点位中心**在哪个方向，
   *     正北=0°、正东=90°、正南=180°、正西=270°（顺时针）。
   *     也可以用手机指南针：站在出发点，读指向签到点位的方位角即可。
   */
  approachBearingDeg: number | null
  /**
   * 进场方向的每次扰动幅度（度）。0 = 固定方向（太整齐，任何两次完全同向也可疑），
   * 默认 35° ⇒ 落点方向在一个 ±35° 的扇形内抖动，既有"固定习惯"又不机械。
   */
...
export const DEFAULT_MORN_SIGN_ZONE: MornSignZone = {
  ...
  innerFraction: 0.15,
  outerFraction: 0.6,
  // ⭐ 默认不预设方向（`null` = 全向随机）——但**这是唯一建议用户去调的参数**，
  //    因为"每次都从同一个方向过来"是真人最强的特征。详见 `approachBearingDeg` 的注释。
